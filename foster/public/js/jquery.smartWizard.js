...
function SmartWizard(target, options) {
    ...
    this.buttons = {
        previous : $('<a class="back_btn btn btn-default btn_round px-0 float-left" href="javascript:;"><i class="fa fa-angle-left"></i>'+options.labelPrevious+'</a>').addClass("buttonPrevious"),
        next : $('<a class="btn btn-primary btn_round form_invalid">'+options.labelNext+'</a>').addClass("buttonNext"),
        finish  : $('<button type="submit" class="btn btn-primary btn_round promot_btn">'+options.labelFinish+'</button>').addClass("buttonFinish")
    };
    ...
    var _init = function($this) {
        var elmActionBar = $('<div class="promo-sec-footer"></div>').addClass("actionBar");
        ...
        $('.input_title_count').change(function(){
            titleVal = $(this).val().length;
            if (titleVal > 0 && imageVal>0) {
                $($this.buttons.next).removeClass('form_invalid')
            }else {
                $($this.buttons.next).addClass('form_invalid')
            }
        })
        $('.dg_dr input[type="file"]').change(function(){
            imageVal = $(this).val().length;
            if (titleVal>0 && imageVal>0) {
                $($this.buttons.next).removeClass('form_invalid')
            } else {
                $($this.buttons.next).addClass('form_invalid')
        ...
        $('#autocomplete').autocomplete({
            ...
            onSelect: function(suggestion,i) {                
                addItem(suggestion);
                if (suggestion !== '' && locationVal !== 0) {
                    $($this.buttons.next).removeClass('form_invalid')
                }
                
                // $('#selction-ajax').html($('#selction-ajax').html() + '<tr id='+suggestion.data+'><td class="tb_lo_cr">'+suggestion.value+'</td><td class="tb_lo_cd">'+suggestion.data+'<span class="delete" onclick="removeItem()" delete-id='+suggestion.data+'><img src="images/ico_cross.png"></span></td></tr>');
        ...
        $($this.buttons.next).click(function() {
            if ($(this).hasClass('form_invalid') !== true) {
                $this.goForward();
